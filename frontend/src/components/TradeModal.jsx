...
function TradeModal({ symbol, currentPrice, stockData, changePercent, isOpen, onClose, isMobile, initialTab = 'BUY' }) {
  ...
  const price = currentPrice?.toFixed(2) ?? '0.00';
  const numericPrice = parseFloat(price) || 0;
  
  const maxBuyShares = numericPrice > 0 ? Math.floor((Number(userBalanceUSD) || 0) / numericPrice) : 0;
  ...
  useEffect(() => {
    if (!isOpen) return;
    // ensure correct default tab each time modal opens
    setActiveTab(initialTab);
    setError('');
    apiGet("/api/balance", { headers: getAuthHeaders() })
      .then(raw => {
        const data = (raw && typeof raw === 'object' && 'data' in raw) ? raw.data : raw;

        if (typeof data === 'number') {
          setUserBalanceUSD(data);
          setUserShares({});
          return;
        }
        ...
        setUserBalanceUSD(Number(usd) || 0);
        setUserShares(shares && typeof shares === 'object' ? shares : {});
      })
      .catch(err => {
  ...
  const handleTradeSubmit = async () => {
    ...
    try {
      const raw = await apiPost("/api/trade", payload, { headers: getAuthHeaders() });
      const data = (raw && typeof raw === 'object' && 'data' in raw) ? raw.data : raw;

      if (data?.error) throw new Error(data.error || 'Trade failed');
  ...
  const isQuantityValid = () => {
    ...
    const maxLimit = activeTab === 'BUY' ? maxBuyShares : Math.floor(availableSharesForSymbol);
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-end gap-2">
            <button
              ...
              onClick={() => setAmount(String(Math.max(1, Math.floor(maxBuyShares / 2))))}
              disabled={maxBuyShares < 1}
            >
              50%
            </button>
            <button
              className="text-xs px-2 py-1 rounded bg-white/5 hover:bg-white/10"
              onClick={() => setAmount(String(maxBuyShares))}
              disabled={maxBuyShares < 1}
            >
              Max
            </button>
          ...
          {activeTab === 'BUY' ? (
            <>
              <div className="flex justify-between items-center">
                ...
                <span>{Number(userBalanceUSD || 0).toFixed(2)} USD</span>
              </div>
              <div className="flex justify-between items-center">
                <span>Max Buy</span>
                <span>{Number.isFinite(maxBuyShares) ? maxBuyShares : 0} {symbol}</span>
              </div>
            </>
          ) : (
            <>
              <div className="flex justify-between items-center">
                ...
                <span>{Number(availableSharesForSymbol || 0).toFixed(4)} {symbol}</span>
              </div>
              <div className="flex justify-between items-center">
                <span>Value (USD)</span>
                <span>${(Number(availableSharesForSymbol || 0) * numericPrice).toFixed(2)}</span>
              </div>
            </>
          )}
        </div>
