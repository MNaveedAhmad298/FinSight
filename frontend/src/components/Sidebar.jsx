...
function Sidebar() {
  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      const response = await apiPost("/api/profile", formData, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } });
  ...
  const handleFundsRequest = async () => {
    ...
    try {
      const response = await apiPost("/api/request-funds", { amount: parseFloat(savedFundsAmount) }, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } });
