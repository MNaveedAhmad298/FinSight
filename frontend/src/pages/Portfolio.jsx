  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
      try {
        ...
        // Use balance endpoint to construct a simple portfolio view
        const data = await apiGet('/api/balance', { headers });
        if (data.error) throw new Error(data.error);

        const usd = data.usd || 0;
        const sharesMap = data.shares || {};

        const holdings = Object.entries(sharesMap).map(([symbol, quantity]) => ({
          symbol,
          name: symbol,
          shares: quantity,
          currentPrice: 0,
