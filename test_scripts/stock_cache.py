...
class StockDataCache:
    def __init__(self, db, SUBSCRIBE_SYMBOLS, periods, cache_dir='./stock_cache'):
        ...
        self.stock_list = SUBSCRIBE_SYMBOLS
    ...
    def get_latest_stocks(self):
        """Return a list of latest stock snapshots for the default short period (1d)."""
        results = []
        default_period = '1d' if '1d' in self.periods else self.periods[0]
        for symbol in self.stock_list:
            try:
                doc = self.db.stock_cache.find_one({'symbol': symbol, 'period': default_period})
                if doc:
                    results.append({
                        'symbol': symbol,
                        'name': symbol,
                        'price': doc.get('currentPrice') or 0,
                        'change': doc.get('changePercent') or 0,
                        'volume': None
                    })
                else:
                    results.append({
